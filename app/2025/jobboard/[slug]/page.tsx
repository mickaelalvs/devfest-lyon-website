...
export default function JobDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen">
      <Script id="job-posting-schema" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(cleanSchema) }} />
      <Header />
        ...
        <div className="container px-4 md:px-6">
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            <Card className={`${colorScheme.border} ${colorScheme.bg} border-4`}>
              <CardHeader>
                <div>
                  <CardTitle className={`text-3xl mb-2 ${colorScheme.title}`}>{job.title}</CardTitle>
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground leading-relaxed whitespace-pre-line">{job.fullDescription || job.description}</p>
                </div>
                ...
                {job.additionalInfo && (
                  <div>
                    ...
                    <p className="text-muted-foreground leading-relaxed whitespace-pre-line">{job.additionalInfo}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
                  ...
                  {job.remote && (
                    <div>
                      ...
                      <p className="font-medium">{typeof job.remote === 'boolean' ? (job.remote ? 'Oui' : 'Non') : job.remote}</p>
                    </div>
                  )}
                  {job.experience && (
                    <div>
                ...
                {(job.contactEmail || job.contactName || job.contactPhone) && (
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-2 text-muted-foreground">
                      {job.contactName && (
                        <p>
                          <span className="font-medium">Nom:</span> {job.contactName}
                        </p>
                      )}
                      {job.contactEmail && (
                        <p>
                ...
                {(job.applyUrl || job.contactEmail) && (
                  <div className="pt-6">
                    <Button asChild size="lg" className={`w-full md:w-auto ${colorScheme.button}`}>
                      {job.contactEmail ? (
                        <a href={`mailto:${job.contactEmail}?subject=Candidature - ${job.title}`}>Postuler maintenant</a>
                      ) : (
                ...
                <div className="flex flex-wrap justify-center gap-6">
                  {sponsor.jobs
                    ...
                    .map((otherJob) => (
                      ...
                      >
                        <CardHeader>
                          <CardTitle className={`text-xl ${colorScheme.title}`}>{otherJob.title}</CardTitle>
                          <CardDescription className="text-base">{otherJob.company}</CardDescription>
                        </CardHeader>
                        <CardContent className="flex-1">
                          <p className="text-sm text-muted-foreground line-clamp-3">{otherJob.description}</p>
                        </CardContent>
                        <CardFooter>
                          <Button asChild variant="outline" className={`w-full ${colorScheme.button} border-current hover:bg-current/10`}>
                            <Link href={`/2025/jobboard/${otherJob.slug}`}>Voir l'offre</Link>
                          </Button>
                        </CardFooter>
                      </Card>
                    ))}
