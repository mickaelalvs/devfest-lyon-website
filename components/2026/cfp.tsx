    ...
    <SectionLayout id="cfp" pill="Appel à conférences" title="Vous avez quelque chose à partager ?" theme="green">
        ...
        >
            ...
            <div className="flex flex-col gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">Si vous souhaitez une session de Q&A, elle devra être comprise dans la durée impartie.</p>
                </div>
                    ...
                    {categories.map(({emoji, label, topics}) => (
                        <div key={label} className="flex flex-col gap-1.5">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {topics.map((topic) => (
                                    <span key={topic} className="rounded-full border border-google-green/30 bg-google-green/5 px-3 py-1 text-xs font-medium text-muted-foreground">
                                        {topic}
                                    </span>
                                ))}
