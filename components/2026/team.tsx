...
import Baptiste from '@/assets/images/team/baptiste-millot.jpeg';
import Mathieu from '@/assets/images/team/mathieu-mure.jpeg';
      ...
      <div className="container px-4 md:px-6">
        ...
        >
          <div className="space-y-2 flex flex-col items-center justify-center gap-2">
            <div className="inline-block rounded-lg bg-green/10 px-3 py-1 text-sm text-green">L'équipe organisatrice</div>
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl bg-clip-text text-green">Rencontrez notre équipe</h2>
          </div>
          <motion.div
            variants={container}
            initial="hidden"
            ...
            {teamMembers.map((member, i) => (
              <motion.div key={i} variants={item} className="flex flex-col items-center space-y-3 group">
                <div className="relative">
                  <div
                    className="relative h-48 w-48 sm:h-36 sm:w-36 md:h-48 md:w-48 overflow-hidden rounded-full bg-background"
                ...
                <div className="space-y-1">
                  <h3 className="leading-none text-foreground font-bold text-2xl" style={{ color: member.color }}>
                    {member.name}
                  </h3>
                </div>
              </motion.div>
