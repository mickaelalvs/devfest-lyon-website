...
const getThemeColors = (theme: string) => {
    const normalizedTheme = theme.replace(/[📝📊🗃️📏🔧]/g, '').trim().toLowerCase();
            ...
            <div className="flex-1">
                <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                        ...
                        {talk.theme ? (
                            <div
                                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getThemeColors(talk.theme)}`}>
                                {talk.theme}
                            </div>
                        ) : null}
                    </div>
